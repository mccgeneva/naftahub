...
export async function loadNqaiChat(userId: string): Promise<StoredNqaiChat> {
  if (!userId) return { messages: [], summary: "", updatedAt: null }
  await ensureTable()
  const { rows } = await query(
    `SELECT messages, summary, updated_at FROM nqai_chats WHERE user_id = $1`,
    [userId],
  )
  if (!rows.length) return { messages: [], summary: "", updatedAt: null }
...
/** Wipe a user's NQAi conversation and memory (used by "New conversation"). */
export async function clearNqaiChat(userId: string): Promise<void> {
  if (!userId) return
  await ensureTable()
  await query(`DELETE FROM nqai_chats WHERE user_id = $1`, [userId])
}
